...
export class GraphPlantUMLPrinter {
    public static generateContents(start: Node, graph: IGraph<unknown>, directional: boolean, addLength: boolean = true) {
        ...
        const steps = graph.getAllPaths(directional).map(edge => {
            return `${edge.from} -> ${edge.to} [label = "${edge.cost}"${addLength ? ', minlen=' + Math.floor(edge.cost / 2) : ''}];`;
        });
        return `@startdot
digraph a {
